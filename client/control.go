...
func (ctl *Control) msgHandler() {
	...
	hbSend := time.NewTicker(time.Duration(g.GlbClientCfg.HeartBeatInterval) * time.Second) // 心跳发送间隔
	defer hbSend.Stop()
	hbCheck := time.NewTicker(time.Second) // Pong 检测间隔
	defer hbCheck.Stop()
	...
	for {
		...
		case rawMsg, ok := <-ctl.readCh:
			...
			case *msg.NewProxyResp:
				ctl.HandleNewProxyResp(m) // 启动代理服务端确认信息
			case *msg.Pong: // 收到服务端心跳确认信息
				ctl.lastPong = time.Now()
...
func (ctl *Control) worker() {
	go ctl.msgHandler() // 消息处理
	go ctl.reader() // 将连接中的数据写入 ctl.readCh
	go ctl.writer() // 将 ctl.sendCh 中的消息写入连接中
