...
func (ctl *Control) Start() {
	...
	go ctl.writer() // 处理 ctl.sendCh 中的数据
	for i := 0; i < ctl.poolCount; i++ {
		ctl.sendCh <- &msg.ReqWorkConn{}
	}

	go ctl.manager() // 处理心跳和 ctl.readCh中的数据
	go ctl.reader() // 将读取到的数据写入 ctl.readCh
	go ctl.stoper()
}
...
func (ctl *Control) GetWorkConn() (workConn net.Conn, err error) {
	...
	default:
		...
		case <-time.After(time.Duration(g.GlbServerCfg.UserConnTimeout) * time.Second): // 超时
